...
function get_val_from_request(){
	...
	function append_q(val){
		if(val.value){query+="&"+val.attributes[1].value+val.value;}
	}
	...
	client.onreadystatechange = function() {		
		...
		for (k in responseArray[0]){
			contentTEXT+="<td><form action=\"javascript:get_val_from_request();\" name=\""+k+"\">"+k+"</br><input id=\""+k+"\" class=\"QUERY\"></form></td>"
		}
		contentTEXT += "</tr>"
